...
const UseCrudCart=()=>{
    ...
    const addProductToCart =data=>{
        ...
        axios.post(url, data , getConfiToken())
        .then(()=>  {
    ...
    const deleteProductFromCard = id => {
        ...
        axios.delete(url, getConfiToken())
    ...
    const updateProductFromCard = (id, data) => {
        const url = `https://e-commerce-api-v2.academlo.tech/api/v1/cart/${id}`
        axios.put(url, data , getConfiToken())
        .then(()=> dispatch(getAllProductsCartThunk()))
        .catch((error)=> console.log(error))
    }

    return {addProductToCart, deleteProductFromCard, updateProductFromCard, error, setError}
}
